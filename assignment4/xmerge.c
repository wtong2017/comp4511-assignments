...
asmlinkage long sys_xmerge(void* args, size_t argslen) {
	...
	// Handle file system
	mm_segment_t old_fs;
	char buf[MAXLEN]; // allocate a buffer for file I/O
	...
	ret = strncpy_from_user(outfileName, outfile, MAXLEN) > 0;
	if (ret < 0) {
		return ret;
	}
	else {
	...
	out_fd = sys_open(outfileName, O_WRONLY | oflags, mode);
	if (out_fd < 0) {
		return out_fd;
	}
	...
	for (i = 0; i < num_files; i++) {
		...
		ret = strncpy_from_user(infileName, infiles[i], PAGE_SIZE) > 0;
		if (ret < 0) {
			return ret;
		}
		...
		while (1) {
			...
			printk(KERN_INFO "%i %i", bytes, strlen(buf));
			sys_write(out_fd, buf, bytes);		
		}
		sys_close(in_fd);
		kfree(infileName);
	}
	...
	// Write the count into user space
	printk("Count: %i\n", count);
	put_user(count, ofile_count); 
